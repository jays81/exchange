...
class OrderParserSpec extends FlatSpec with Matchers {
  ...
  "OrderParse" should "parse a valid sell order" in {
    ...
    val parsedOrder = orderParser.parseOrder(Some(orderToParse), "User1")

    parsedOrder should be (Some(Order(Direction.sell, "VOD.L", 1000, 100.2, "User1")))
  }
  ...
  it should "parse a valid buy order" in {
    ...
    parsedOrder should be (Some(Order(Direction.buy, "VOD.L", 1000, 99, "User1")))
  }
